...
const ContentManager = () => {
  ...
  useEffect(() => {
    const fetchArticles = async () => {
      setIsLoading(true);
      try {
        const response = await axios.get('https://preview--gulf-dashboard-admin.lovable.app/api/articles');
  ...
  useEffect(() => {
    const fetchResources = async () => {
      setIsLoading(true);
      try {
        const response = await axios.get('https://preview--gulf-dashboard-admin.lovable.app/api/ressources');
  ...
  useEffect(() => {
    const fetchInitiatives = async () => {
      setIsLoading(true);
      try {
        const response = await axios.get('https://preview--gulf-dashboard-admin.lovable.app/api/initiatives/get-all-initiatives');
        const data = response.data;
        const mapped = data.map((res: any) => ({
          id: res.id?.toString() || Date.now().toString(),
          title: res.title,
  ...
  const handleCreateItem = async (data: any) => {
    ...
    if (selectedTab === 'resources') {
      ...
      try {
        ...
        const response = await axios.post('https://preview--gulf-dashboard-admin.lovable.app/api/ressources/upload', formData, {
    ...
    if (selectedTab === 'projects') {
      ...
      try {
        ...
        if (data.file) {
          ...
          const uploadResponse = await axios.post('https://preview--gulf-dashboard-admin.lovable.app/api/initiatives/upload-image-initiative', imageData, {
            headers: { 'Content-Type': 'multipart/form-data' },
          });
          imageUrl = uploadResponse.data.imageUrl;
        }

        const response = await axios.post('https://preview--gulf-dashboard-admin.lovable.app/api/initiatives/create-new-initiative', {
          title: data.title,
    ...
    try {
      ...
      if (data.file) {
        const imageData = new FormData();
        imageData.append('file', data.file);
        const uploadResponse = await axios.post('https://preview--gulf-dashboard-admin.lovable.app/api/articles/upload-image', imageData, {
          headers: { 'Content-Type': 'multipart/form-data' },
        });
        imageUrl = uploadResponse.data.imageUrl;
      }

      const response = await axios.post('https://preview--gulf-dashboard-admin.lovable.app/api/articles', {
        title: data.title,
  ...
  const handleDeleteItem = async (item: ContentItem) => {
    try {
      if (item.type === 'article') {
        await axios.delete(`https://preview--gulf-dashboard-admin.lovable.app/api/articles/${item.id}`);
        setArticles(articles.filter((a) => a.id !== item.id));
      } else if (item.type === 'resource') {
        await axios.delete(`https://preview--gulf-dashboard-admin.lovable.app/api/ressources/${item.id}`);
        setResources(resources.filter((r) => r.id !== item.id));
      } else if (item.type === 'project') {
        await axios.delete(`https://preview--gulf-dashboard-admin.lovable.app/api/initiatives/delete-initiative/${item.id}`);
